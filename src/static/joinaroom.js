...
function join_room_window(name, pwd_req) {
    ...
    if (pwd_req) {
        $('#join').before('<input id="room_pwd" type="password" ' +
    ...
    if (data.data.length === 0) {
        $('#room_container')
            .append('<button class="btn btn-default ' +
                'btn-primary btn-block">Create a Room</button>')
        ...
        $.each(data.data, function(index, value) {
            var button = $($.parseHTML('<button class="btn btn-default ' +
                'btn-primary btn-block">' + value.name + '</button>'));
